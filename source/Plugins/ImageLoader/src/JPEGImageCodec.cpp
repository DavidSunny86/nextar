...
boolean JpegFillInputBuffer(j_decompress_ptr cinfo) {
	...
	if (manager->inpSrc->Read(manager->readBuffer, bytesToRead)
			== bytesToRead) {
...
ImageData JPEGImageCodec::Load(InputStreamPtr& stream,
	...
	::byte* output = (::byte*) NEX_ALLOC(sizeof(::byte) * rowspan,
			MEMCAT_GENERAL);
...
void JPEGImageCodec::Save(OutputStreamPtr& file, const ImageParams& params,
