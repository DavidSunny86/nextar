...
class RenderContext_Base_GL :
  ...
  public CoreFuncTableVersion32 {
    typedef BaseExtensionsGL Extensions;
    typedef FuncTable_32 FunctionTable;
...
public:
  ...
  inline void ClientWaitSync(GLsync sync​, GLbitfield flags​, GLuint64 timeout​);
...
protected:
  ...
  static GLenum s_colorAttachments[RenderConstants::MAX_COLOR_TARGETS];
};
...
void RenderContext_Base_GL::CreateFBO(RenderTextureViewGL* view) {
  CreateFBO(view->fbo);
  BindWriteFBO(view->fbo);
  AttachToFBO(view, view->pixelFormat.attachmentType + 0);
  NEX_ASSERT(ValidateFBO());
  UnbindWriteFBO();
}
...
inline void RenderContext_Base_GL::ClientWaitSync(GLsync sync​, GLbitfield flags​,
   GLuint64 timeout​) {
