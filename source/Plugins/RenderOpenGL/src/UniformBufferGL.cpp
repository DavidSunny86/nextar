...
namespace RenderOpenGL {
	...
	void UniformBufferGL::BeginUpdate(RenderContext* rc, uint32 updateFlags) {
		...
		if(!IsCustomStruct()) {
			mappedMem =
					static_cast<uint8*>(
					gl->MapRange(GL_UNIFORM_BUFFER, 0, GetSize(), GL_MAP_WRITE_BIT|GL_MAP_INVALIDATE_BUFFER_BIT));
		}
	}
	...
	void UniformBufferGL::Read(RenderContext* rc, void *dest, size_t offset = 0, size_t size = 0) {
		NEX_THROW_FatalError(EXCEPT_NOT_IMPLEMENTED);
	}

	void UniformBufferGL::Write(RenderContext* rc, const void *src, size_t offset, size_t size) {
		RenderContextGL* gl = static_cast<RenderContextGL*>(rc);
		if (mappedMem) {
