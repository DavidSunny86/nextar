...
IndexBufferGL::IndexBufferGL(size_t sz, uint32 flags, IndexBuffer::Type type) :
		IndexBuffer(sz, flags, type), bufferId(0) {
...
void IndexBufferGL::Read(RenderContext* rc, void *dest, size_t offset,
		size_t size) {
	RenderContextGL* rcGL = static_cast<RenderContextGL*>(rc);
	if (bufferId) {
...
void IndexBufferGL::Write(RenderContext* rc, const void *src, size_t offset,
		size_t size) {
	if (!bufferId)
		_Create(rc);
	RenderContextGL* rcGL = static_cast<RenderContextGL*>(rc);
	rcGL->Bind(GL_ELEMENT_ARRAY_BUFFER, bufferId);
	rcGL->WriteBuffer(GL_ELEMENT_ARRAY_BUFFER, GetSize(), usage, src, offset,
			size);
}
