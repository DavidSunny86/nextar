...
namespace RenderOpenGL {
	...
	PassViewGL::PassViewGL() : inputSemantics(nullptr)
	,iGlProgram(0)
	,inputLayoutId(-1) {
	}
	...
	void PassViewGL::Compile(nextar::RenderContext* rc,
		...
		for(nextar::uint32 i = 0; i < Pass::STAGE_COUNT; ++i) {
			if(params.programSources[i].length() > 1) {
				...
				if(!shader.Compile(stagesMap[i], rc,
						...
						params.compileOptions ? *params.compileOptions :
								StringUtils::Null)) {
					Error("Program compilation failed due to failed shader compilation!");
					return;
				}
		...
		bool produceReflectionData = params.inited.compare_exchange_strong(preCompileStatus, 1,
				std::memory_order_release,
				std::memory_order_relaxed);

		// read uniform data and register to uniform buffer manager
		ctx->ReadUniforms(this, params.passIndex, iGlProgram, produceReflectionData ? params.parameters : nullptr);
		// read samplers
		ctx->ReadSamplers(this, params.passIndex, iGlProgram, produceReflectionData ? params.parameters : nullptr,
	...
	void PassViewGL::SetTexture(RenderContext* rc, const SamplerParameter& desc,
		...
		TextureViewGL* texture = static_cast<TextureViewGL*>(ctx->GetView(tu->texture));
	...
	std::pair<uint16, VertexSemanticListGL*> PassViewGL::MapLayout(const VertexSemanticGL* semantics,
			uint32 numSemantics) {
