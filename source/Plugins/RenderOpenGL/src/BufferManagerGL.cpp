...
BufferManagerGL::BufferManagerGL(size_t memoryBudget, uint32 rft) :
		BufferManager(memoryBudget, rft) {
}
...
VertexBufferPtr BufferManagerGL::CreateVertexBuffer(size_t bufferSize,
		uint32 accessFlags, RelocationPolicy policy) {

	VertexBufferGL* vb = NEX_NEW(
			VertexBufferGL(bufferSize, accessFlags, policy));
...
IndexBufferPtr BufferManagerGL::CreateIndexBuffer(size_t bufferSize,
...
void BufferManagerGL::FreeUnusedBuffers() {
	...
	for (TemporaryBufferPool::iterator it = tempBuffers.begin(); en != it;) {
