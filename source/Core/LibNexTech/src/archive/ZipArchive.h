...
//! is written.
		_NexZipApi ZipFile* AddNewFileToArchive(ZipArchive*,
				const char* filename, size_t buffsize, bool pack);
...
//! delete existing file from archive
		_NexZipApi bool DeleteFileFromArchive(ZipArchive*,
				const char* filename);
//! file exists
...
//! write
		_NexZipApi bool WriteToArchiveFile(ZipArchive*, ZipFile*, const char*,
				size_t);
//! get/set file time
...
//! tell
		...
		public:
			...
			ZipArchive() :
					numRefs(1), flags(0), comment(0), commentLength(0), delHead(
							0), entriesAdded(0), entriesHead(0), entriesTail(0), numFiles(
							0) {
			}
