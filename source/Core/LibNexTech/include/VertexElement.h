...
namespace nextar {
	...
	struct _NexExport VertexSemantic: public AllocGraphics {
	...
	struct VertexDesc: public AllocGraphics {
		...
		uint16 offset;
		/* Vertex stride, such that skipping this amount of bytes
		 * will reach the next vertex with this element. */
		uint16 stride;
		...
		inline VertexDesc(const VertexSemantic sem, uint16 _offset,
				uint16 _stride) :
				semantic(sem), offset(_offset), stride(_stride) {
