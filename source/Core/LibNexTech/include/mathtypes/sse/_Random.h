...
#define NEX_MATH_RANDOM_SEED _NexExport NEX_EFFICIENT_ALIGN(__m128i nextar::MathRandomizer::curSeed);

class MathRandomizer {
	_NexExport NEX_EFFICIENT_ALIGN(static __m128i curSeed);
public:

	static _NexInline void Seed(unsigned int seed) {
		curSeed = _mm_set_epi32(seed, seed + 1, seed, seed + 1);
	}

	static _NexInline void RandSSE(unsigned int* result) {
