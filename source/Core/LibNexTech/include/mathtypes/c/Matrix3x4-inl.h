namespace nextar {
	...
	inline Matrix3x4 Mat3x4FromVectorMapping(const Vector3& axis1,
		...
		cs = Dot(axis1, axis2);
		{
			...
			if (fabsf(axis.x) < Math::EPSILON_MED && fabsf(axis.y) < Math::EPSILON_MED
					&& fabsf(axis.z) < Math::EPSILON_MED) {
				// take a cross for that
				axis = Cross(axis1, Vector3::YAxis);
				if (fabsf(axis.x) < Math::EPSILON_MED
						&& fabsf(axis.y) < Math::EPSILON_MED
						&& fabsf(axis.z) < Math::EPSILON_MED) {
