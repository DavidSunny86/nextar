...
// name
		...
		1, 1, PIXFLAG_INTEGRAL | PIXFLAG_HAS_ALPHA | PIXFLAG_TEXTURE_FORMAT
...
// name
		"RGBA8",
		// pixelSize, numChannels, flags
		4, 4, PIXFLAG_INTEGRAL | PIXFLAG_HAS_ALPHA | PIXFLAG_TEXTURE_FORMAT
				| PIXFLAG_UNSIGNED | PIXFLAG_NORMALIZED },
...
{ "D24S8",
// pixelSize, numChannels, flags
		4, 2, PIXFLAG_FLOAT | PIXFLAG_TEXTURE_FORMAT | PIXFLAG_UNSIGNED
				| PIXFLAG_NORMALIZED | PIXFLAG_DEPTH | PIXFLAG_STENCIL },

{ "D32",
// pixelSize, numChannels, flags
		4, 1, PIXFLAG_FLOAT | PIXFLAG_TEXTURE_FORMAT | PIXFLAG_UNSIGNED
				| PIXFLAG_NORMALIZED | PIXFLAG_DEPTH },
...
static void _PixelTransferImpl(PixelBox& dest,
		const PixelBox& src) {
...
static void _PackPixel(ColorChannel& c, const void *data, PixelFormat fmt) {
...
/* Pixel transfer */
_NexEngineAPI void PixelTransfer(PixelBox& dest, const PixelBox& src,
		Filter filter) {
	/* do some validation*/
	/* end of validation */
	...
	if (src.format != dest.format) {
		if (needsResize) {
			freeProxySrc = true;
			proxySrc.data = (uint8*) NEX_ALLOC(
					GetBufferSize(proxySrc.GetWidth(), proxySrc.GetHeight(),
		...
		if (src.IsSimpleArray() && dest.IsSimpleArray()) {
			...
			for (uint32 d = src.front; d < src.back; ++d) {
				...
				for (uint32 h = src.top; h < src.bottom; ++h) {
					...
					std::memcpy(destRow, srcRow, width * bytesPerPix);
				}
			}
		}
	}
...
_NexEngineAPI void ResizeBox(PixelBox& dest, const PixelBox& src,
		Filter filter) {
	switch (filter) {
