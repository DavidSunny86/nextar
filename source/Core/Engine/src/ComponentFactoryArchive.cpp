...
namespace nextar {
	...
	void ComponentFactoryArchive::AsyncRemoveFactory(uint32 type, StringID name, bool d) {
	...
	Component::Factory* ComponentFactoryArchive::AsyncFindFactory(uint32 type, const StringID name) {
		NEX_THREAD_LOCK_GUARD_MUTEX(containerLock);
		ComponentFactoryMap::iterator it = componentFactories.find(FactoryID(name, type));
		if ( it != componentFactories.end() )
			return (*it).second;
		return 0;
	}
