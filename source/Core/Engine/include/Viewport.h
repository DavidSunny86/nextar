...
namespace nextar {
	...
	public:
		...
		public:
			...
			friend inline bool operator ==(const Iterator it1,
					const Iterator it2) {
		...
		Viewport(Camera *cam, RenderTarget *rt, float x = 0.f, float y = 0.f,
				float width = 1.f, float height = 1.f, int32 priority = 0, uint32 flags =
						ACTIVE, const String& optName = StringUtils::Null, Viewport* next = 0);
		virtual ~Viewport();
