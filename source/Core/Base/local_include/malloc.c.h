...
/* RefPtr to next or previous physical malloc_chunk. */
...
#if !ONLY_MSPACES
	/* Set up lock for main malloc area */gm->mflags = mparams.default_mflags;
...
/* set size of fake trailing chunk holding overhead space only once */chunk_plus_offset(p, psize)->head =
TOP_FOOT_SIZE;
...
if (!PREACTION(m)) {
	...
	if (RTCHECK(
			ok_address(m, oldp) && ok_cinuse(oldp) && ok_next(oldp, next) && ok_pinuse(next))) {
...
if (bytes >= MAX_REQUEST - alignment) {
	...
	if (mem != 0) {
		...
		assert (chunksize(p) >= nb);assert((((size_t)(chunk2mem(p))) % alignment) == 0);check_inuse_chunk(m, p);POSTACTION(m);
		if (leader != 0) {
