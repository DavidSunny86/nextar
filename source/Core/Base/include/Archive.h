...
struct FileAttribute: public AllocGeneral {
	...
	enum Flags {
		...
		ATTRIB_COMPRESSED = 1 << 6, ATTRIB_DEFAULTS = 0
	};
 ...
 * @date	11/28/2009
 **/
class _NexBaseAPI Archive: public Referenced<Archive, AllocGeneral> {
...
public:
	...
	struct ScanCallback {
		...
		virtual void FoundFile(const FileAttribute& attribute, Archive*) = 0;
	protected:
		~ScanCallback() {
		}
	};
