...
inline void RadixSort(_It* a, uint32 size, Integer bitMask,
		uint32 shiftRightAmount) {
	...
	if (bitMask != 0) {
		...
		for (uint32 i = 0; i < numberOfBins; i++) {
			...
			if (numberOfElements >= Threshold)
				RadixSort<_It, Threshold, Integer>(&a[startOfBin[i]],
						numberOfElements - 1, bitMask, shiftRightAmount);
