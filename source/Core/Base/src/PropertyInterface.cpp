...
public:
	...
	PropertyDictionary* AsyncGetDictionary(PropertyDictionary* parent,
			const String& name) {
		...
		PropertyDictionary* dictionary = NEX_NEW(
				PropertyDictionary(name, parent));
...
void PropertyInterface::SetParamValue(const String& param,
		const String& value) {
...
bool PropertyInterface::OverrideDictionary(const String& name) {
	if (dictionary && dictionary->name == name)
		return false;
	dictionary =
			PropertyDictionaryManager::propertyDictionaryManager.AsyncGetDictionary(
					dictionary, name);
	return true;
}
