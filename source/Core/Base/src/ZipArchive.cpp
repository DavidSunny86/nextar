...
static bool ReadExtraField(ZipFile* zipf, InputStreamPtr& infile,
	...
	if (zipf->extrafield && (zipf->info.extraFieldLength != extraFieldLength)) {
	...
	if (extraFieldLength) {
		if (!zipf->extrafield)
			zipf->extrafield = (char*) NEX_ALLOC(extraFieldLength,
					MEMCAT_GENERAL);
...
static bool ReadFileComment(ZipFile* zipf, InputStreamPtr& infile,
	...
	if (zipf->comment && (zipf->info.fileCommentLength != fileCommentLength)) {
	...
	if (fileCommentLength) {
		if (!zipf->comment)
			zipf->comment = (char*) NEX_ALLOC(fileCommentLength,
					MEMCAT_GENERAL);
...
static bool ReadLFH(zLocalFileHdr *lfh, InputStreamPtr& infile) {
	...
	lfh->versionNeededToExtract[0] = buff[L_VERSION_NEEDED_TO_EXTRACT_0];
	lfh->versionNeededToExtract[1] = buff[L_VERSION_NEEDED_TO_EXTRACT_1];
	lfh->generalPurposeBitFlag = BUFF_GET_SHORT(L_GENERAL_PURPOSE_BIT_FLAG);
...
static bool ReadCDFH(zCentralDirFileHdr* cdfh, InputStreamPtr& infile) {
	...
	cdfh->versionNeededToExtract[0] = buff[C_VERSION_NEEDED_TO_EXTRACT_0];
	cdfh->versionNeededToExtract[1] = buff[C_VERSION_NEEDED_TO_EXTRACT_1];
	cdfh->generalPurposeBitFlag = BUFF_GET_SHORT(C_GENERAL_PURPOSE_BIT_FLAG);
	...
	cdfh->internalFileAttributes = BUFF_GET_SHORT(C_INTERNAL_FILE_ATTRIBUTES);
	cdfh->externalFileAttributes = BUFF_GET_LONG(C_EXTERNAL_FILE_ATTRIBUTES);
	cdfh->relativeOffsetLocalHeader = BUFF_GET_LONG(
...
static void LoadECDR(zEndCentralDirRecord* ecdr, char *buff) {
	...
	ecdr->numDiskStartCdir = BUFF_GET_SHORT(E_NUM_DISK_WITH_START_CENTRAL_DIR);
	ecdr->numEntriesCentrlDirThsDisk = BUFF_GET_SHORT(
			E_NUM_ENTRIES_CENTRL_DIR_THS_DISK);
	ecdr->totalEntriesCentralDir = BUFF_GET_SHORT(E_TOTAL_ENTRIES_CENTRAL_DIR);
	ecdr->sizeCentralDirectory = BUFF_GET_LONG(E_SIZE_CENTRAL_DIRECTORY);
	ecdr->offsetStartCentralDirectory = BUFF_GET_LONG(
			E_OFFSET_START_CENTRAL_DIRECTORY);
	ecdr->zipfileCommentLength = BUFF_GET_SHORT(E_ZIPFILE_COMMENT_LENGTH);
}
...
static bool ReadArchiveComment(ZipArchive* ar, InputStreamPtr& infile,
	...
	if (!ar->comment)
		ar->comment = (char*) (char*) NEX_ALLOC(commentLength, MEMCAT_GENERAL);
	return (infile->Read(ar->comment, commentLength) == commentLength) != 0;
}
...
static void ReadZipEntries(ZipArchive* arch, InputStreamPtr& infile) {
	...
	while ((infile->Read(buff, sizeof(hdrLocal)) >= sizeof(hdrLocal))
			...
			&& (ReadLFH(&lfh, infile))) {
		newOffs = curOffs + sizeof(hdrLocal) + ZIP_LOCAL_FILE_HEADER_SIZE
				+ lfh.filenameLength + lfh.extraFieldLength + lfh.csize;
		if ((lfh.filenameLength >= sizeof(buff))
				|| (infile->Read(buff, lfh.filenameLength) < lfh.filenameLength))
		...
		if ((buff[lfh.filenameLength - 1] != '/')) {
			...
			cdfh.versionNeededToExtract[0] = lfh.versionNeededToExtract[0];
			cdfh.versionNeededToExtract[1] = lfh.versionNeededToExtract[1];
...
static ZipArchive* ReadZipDir(InputStreamPtr& infile) {
	...
	ZipArchive* arch = 0;
	size_t step = ZIP_END_CENTRAL_DIR_RECORD_SIZE + sizeof(hdrEndcentral);
	...
	if (curOffs
			>= (65535 + ZIP_END_CENTRAL_DIR_RECORD_SIZE + sizeof(hdrEndcentral)))
		minOffs = curOffs
			...
			for (searchPtr = &buff[searchPos - step]; searchPtr > buff;
				...
				if ((*searchPtr == 'P')
					...
					if (!infile->Seek(
							...
							|| !infile->Seek(ecdr.offsetStartCentralDirectory,
									std::ios_base::beg))
						goto rebuildCdr;
					...
					for (;;) {
						...
						if ((infile->Read(buff, sizeof(hdrCentral))
								< sizeof(hdrCentral))
								|| (memcmp(buff, hdrCentral, sizeof(hdrCentral))
										!= 0)) {
							if (arch->numFiles)
						...
						if ((!ReadCDFH(&cdfh, infile))
								...
								|| (infile->Read(buff, cdfh.filenameLength)
										< cdfh.filenameLength))
...
static bool WriteLFH(ZipFile* thisFile, OutputStreamPtr& outfile) {
	...
	buff[L_VERSION_NEEDED_TO_EXTRACT_0] = info->versionNeededToExtract[0];
	buff[L_VERSION_NEEDED_TO_EXTRACT_1] = info->versionNeededToExtract[1];
	BUFF_SET_SHORT(L_GENERAL_PURPOSE_BIT_FLAG, info->generalPurposeBitFlag);
	...
	info->filenameLength = (ush) StringUtils::Length(thisFile->filename);
	BUFF_SET_SHORT(L_FILENAME_LENGTH, info->filenameLength);
	info->extraFieldLength = thisFile->extrafield ? info->extraFieldLength : 0;
...
static bool WriteCDFH(ZipFile* file, OutputStreamPtr& outfile) {
	...
	buff[C_VERSION_NEEDED_TO_EXTRACT_0] = info->versionNeededToExtract[0];
	buff[C_VERSION_NEEDED_TO_EXTRACT_1] = info->versionNeededToExtract[1];

	BUFF_SET_SHORT(C_GENERAL_PURPOSE_BIT_FLAG, info->generalPurposeBitFlag);
	...
	info->fileCommentLength = file->comment ? info->fileCommentLength : 0;
	BUFF_SET_SHORT(C_FILE_COMMENT_LENGTH, info->fileCommentLength);
	BUFF_SET_SHORT(C_DISK_NUMBER_START, info->diskNumberStart);
	BUFF_SET_SHORT(C_INTERNAL_FILE_ATTRIBUTES, info->internalFileAttributes);
	BUFF_SET_LONG(C_EXTERNAL_FILE_ATTRIBUTES, info->externalFileAttributes);
...
static bool WriteECDR(ZipArchive* arch, zEndCentralDirRecord* ecdr,
	...
	BUFF_SET_SHORT(E_NUM_DISK_WITH_START_CENTRAL_DIR, ecdr->numDiskStartCdir);
	BUFF_SET_SHORT(E_NUM_ENTRIES_CENTRL_DIR_THS_DISK,
			ecdr->numEntriesCentrlDirThsDisk);
	BUFF_SET_SHORT(E_TOTAL_ENTRIES_CENTRAL_DIR, ecdr->totalEntriesCentralDir);
	...
	BUFF_SET_SHORT(E_ZIPFILE_COMMENT_LENGTH, ecdr->zipfileCommentLength);
...
static bool WriteFile(ZipFile* file, OutputStreamPtr& outfile) {
	...
	while (!finished) {
		if (!outfile->Seek(
				(long) (lfhoffs + sizeof(hdrLocal) + ZIP_LOCAL_FILE_HEADER_SIZE
						+ StringUtils::Length(file->filename)
						+ (file->extrafield ? file->info.extraFieldLength : 0)),
				std::ios_base::beg))
			return false;
		...
		case ZIP_STORE: {
			...
			file->info.csize = file->info.ucsize = (uint32) file->bufferPos;
			finished = true;
			break;
		}
		case ZIP_DEFLATE: {
...
static bool WriteCentralDirectory(ZipArchive* arch, OutputStreamPtr& temp) {
	size_t count = 0;
	ZipFile* f;
...
static ZipFile* GetArchiveAddedFile(ZipArchive* arch, const char* name) {
...
static bool WriteZipArchive(ZipArchive* arch) {
	...
	while (file->Read(buff, sizeof(hdrLocal)) == sizeof(hdrLocal)) {
		...
		if (memcmp(buff, hdrLocal, sizeof(hdrLocal)) == 0) {
			...
			if (!ReadLFH(&lfh, file))
				goto tempFailed;
			thisName = (char*) NEX_ALLOC(lfh.filenameLength + 1,
					MEMCAT_GENERAL);
			if (file->Read(thisName, lfh.filenameLength) < lfh.filenameLength) {
			...
			if (IsDeleted(arch, thisName)) {
				skipEntry: bytesToSkip = lfh.extraFieldLength + lfh.csize;
				bytesToCopy = 0;
				NEX_FREE(thisName, MEMCAT_GENERAL);
			} else {
			...
			bytesToSkip = cdfh.filenameLength + cdfh.extraFieldLength
					+ cdfh.fileCommentLength;
		} else if (memcmp(buff, hdrEndcentral, sizeof(hdrEndcentral)) == 0) {
	...
	/* disk space for yet another copy of the archive :-( */
	{
		...
		if (tempFile) {
			...
			OutputStreamPtr outFile = fileSys.OpenWrite(arch->fileName);
			InputStreamPtr inpFile = Assign(NEX_NEW(FileInputStream(tempFile)));
			if (outFile)
...
_NexZipApi ZipFile* FindArchiveFile(ZipArchive* arch, const char* name) {
	ZipFile* f;
...
_NexZipApi ZipFile* AddNewFileToArchive(ZipArchive* arch, const char* filename,
		size_t size, bool pack) {
	ZipFile* zf;
...
_NexZipApi bool DeleteFileFromArchive(ZipArchive* arch, const char* filename) {
	if (GetArchiveDirFile(arch, filename)) {
		StrNode* n = (StrNode*) NEX_ALLOC(sizeof(StrNode), MEMCAT_GENERAL);
...
_NexZipApi bool FileExistsInArchive(ZipArchive* arch, const char* filename) {
	if (GetArchiveDirFile(arch, filename))
		return true;
...
static char* ReadEntry(InputStreamPtr& infile, ZipFile * f) {
	...
	if ((!infile->Seek(f->info.relativeOffsetLocalHeader, std::ios_base::beg))
	...
	case ZIP_DEFLATE: {
		...
		while (bytesLeft) {
			...
			err = inflate(&zs, bytesLeft > size ? Z_PARTIAL_FLUSH : Z_FINISH);
			bytesLeft -= size;
		} /* endwhile */
		inflateEnd(&zs);
...
_NexZipApi char* ReadFromArchiveFile(ZipArchive* a, ZipFile* f, size_t* s) {
	if (!f)
		return 0;
	if (s)
		*s = f->info.ucsize;
...
_NexZipApi bool WriteToArchiveFile(ZipArchive* a, ZipFile* f, const char* data,
		size_t s) {
...
static void PackTime(const FileTime* tm, ush* rdate, ush* rtime) {
	*rdate = (ush) ((((int32) (tm->year - 1980) & 0x7f) << 9)
			| (((int32) (tm->mon & 0x0f) + 1) << 5) | (int32) (tm->day & 0x1f));
	*rtime =
			(ush) (((int32) (tm->hrs & 0x1f) << 11)
					| ((int32) (tm->min & 0x3f) << 5)
					| ((int32) (tm->sec >> 1) & 0x1f));
}
...
_NexZipApi void GetArchiveFileTime(ZipFile* f, FileTime* tm) {
	if (f) {
		UnpackTime(f->info.lastModFileDate, f->info.lastModFileTime, tm);
	}
}

_NexZipApi void SetArchiveFileTime(ZipFile* f, const FileTime* tm) {
	if (f) {
		PackTime(tm, &(f->info.lastModFileDate), &(f->info.lastModFileTime));
	}
}

_NexZipApi bool AppendToReadArchiveFile(ZipArchive* arch, ZipFile* file) {
