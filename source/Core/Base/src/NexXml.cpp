...
static const char* ReadText(const char* parse_data, String &text_str,
		const char* end_tag, bool ignore_case);
...
public:
	...
	IncludeNode(Document* doc) :
			Node(doc), root_element(0) {
	}
...
class ConditionBlock: public Node {
	...
	;
	typedef vector<Node*>::type NodePtrArray;bool is_true;
	NodePtrArray added_nodes;
public:

	ConditionBlock(Document* doc) :
			Node(doc), is_true(false) {
	}
...
public:
	...
	ConditionNode(Document* doc) :
			Node(doc), true_node(0) {
	}
...
const String& Element::GetAttributeValue(const char* name,
...
void Document::Load(InputStreamPtr& stream) {
	DataPtr data(TryLoad<MEMCAT_GENERAL>(stream));
	if (!ParseData(reinterpret_cast<const char*>(data.get())))
...
void PrintAttribute(std::ostream& writer, const String& name,
		const String& value) {
...
const char* ParseAttributeData(const char * parse_data, String& name,
		String& value) {
...
void Element::Print(std::ostream& writer, int32 depth) {
	...
	for (NameValueMap::iterator it = attributes.begin(); it != attributes.end();
			++it) {
		writer << " ";
...
const char* ConditionBlock::ParseData(const char* p) {
	...
	p = StringUtils::EatWhite(p);
	while (p && *p) {
		if (StringEqual(p, "<?else", false) || StringEqual(p, "<?elif", false)
...
const char* IncludeNode::ParseData(const char* p) {
	...
	p++;
	{
		...
		if (!f.is_open())
			Warn(
					String(
							"XML Document referred in include node was not found: ")
							+ value);
		else {
			GenericInputStream strm(&f);
			InputStreamPtr inp_ptr = Assign<InputStream>(&strm);
			UniquePtr<uint8>::type newp(TryLoad<MEMCAT_GENERAL>(inp_ptr));
			if (newp)
				ReadValue(reinterpret_cast<char*>(newp.get()), "");
			Node* n = FirstChild();
			if (n) {
				if (n->Next()) {
					Error(
							String(
									"XML Document in include node needs a single root: ")
									+ value);
					return 0;
				}
				root_element = n;
...
static BasicEntity entities[] = { { ("&amp;"), 5, '&' }, { ("&lt;"), 4, '<' }, {
		("&gt;"), 4, '>' }, { ("&quot;"), 6, '\"' }, { ("&apos;"), 6, '\'' } };
...
static const char* ReadName(const char* parse_data, String& name_str) {
	...
	if (parse_data && *parse_data
			&& (isalpha((char) *parse_data) || *parse_data == ('_'))) {
		while (*parse_data
				&& (isalnum((char) *parse_data) || *parse_data == ('_')
...
static const char* ReadText(const char* parse_data, String &text_str,
		const char* end_tag, bool ignore_case) {
	text_str = "";
	while (parse_data && *parse_data
			&& !StringEqual(parse_data, end_tag, ignore_case)) {
		char c;
