...
bool MemoryOutputStream::Seek(std::streamoff offset,
	...
	case std::ios_base::cur:
		pos = std::max<int32>(
				(int32) std::min<int32>((int32) pos + (int32) offset,
						(int32) size), 0);
		break;
	case std::ios_base::end:
		pos = (offset < 0) ?
...
//--------------------------------------------------------
GenericInputStream::GenericInputStream(std::istream* stream, size_t streamSize,
		bool bfree) :
...
//--------------------------------------------------------
GenericOutputStream::GenericOutputStream(std::ostream* strm, size_t streamSize,
		bool bfreeStream) :
...
bool FileInputStream::Seek(std::streamoff offset, std::ios_base::seekdir dir) {
...
bool FileOutputStream::Seek(std::streamoff offset, std::ios_base::seekdir dir) {
	return std::fseek(streamptr, (long) offset, dir) == 0;
}
