...
namespace nextar {
	...
	StringInternTable::StringInternTable(const String& _name) : loadedFromCache(false),
			saveToCache(false), name(_name) {
	...
	URL StringInternTable::_GetURL() {
		return URL("{" + FileSystem::ArchiveEngineData + "}/StringTable_" + name + ".table");
	}
	...
	void StringInternTable::_LoadFromCache() {
		...
		auto it = forwardMap.insert(ForwardMap::value_type(StringUtils::NullID, StringUtils::Null)).first;
		reverseMap.emplace(std::cref((*it).second), (*it).first);
		it = forwardMap.insert(ForwardMap::value_type(StringUtils::DefaultID, StringUtils::Default)).first;
		reverseMap.emplace(std::cref((*it).second), (*it).first);
