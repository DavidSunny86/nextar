...
//@ this may look bad
		...
		static long GetTicket(StringMap& smap, RevStringMap& rmap,
				const char* name) {
...
// now we do not do that
		...
		void OnNewDbg(void* block, size_t size, const char* functiOn,
		...
		void CheckLeaks(std::ostream& outstream) {
			...
			if (blockHead) {
				...
				while (mblock) {
					RevStringMap::iterator fn = revfuncmap.find(
							mblock->functiOn);
					RevStringMap::iterator fl = revfilemap.find(
							mblock->srcfile);

					outstream << _IntfName << ": " << std::endl
							<< "  Leak: Address = " << (size_t) mblock->block
							<< std::endl << "  Func = " << (*fn).second
							<< std::endl << "  File = " << (*fl).second
							<< std::endl << "  Line = " << mblock->line
							<< std::endl;
					mblock = mblock->next;
				}
